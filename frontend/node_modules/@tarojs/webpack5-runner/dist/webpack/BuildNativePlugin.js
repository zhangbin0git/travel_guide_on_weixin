...
class BuildNativePlugin {
    constructor(combination) {
        this.commonChunks = ['runtime', 'vendors', 'taro', 'common'];
    ...
    init() {
        ...
        const pluginDir = node_path_1.default.join(sourceDir, 'plugin');
        const pluginConfigPath = node_path_1.default.join(pluginDir, 'plugin.json');
        ...
        Object.keys(pluginConfig).forEach(key => {
            if (key === 'main') {
                const filePath = node_path_1.default.join(pluginDir, pluginConfig[key]);
                const fileName = node_path_1.default.basename(filePath).replace(node_path_1.default.extname(filePath), '');
                pluginMainEntry = `${fileName}`;
                entryObj[pluginMainEntry] = [(0, helper_1.resolveMainFilePath)(filePath.replace(node_path_1.default.extname(filePath), ''))];
            }
            else if (key === 'publicComponents' || key === 'pages') {
                Object.keys(pluginConfig[key]).forEach(subKey => {
                    const filePath = node_path_1.default.join(pluginDir, pluginConfig[key][subKey]);
                    entryObj[`${pluginConfig[key][subKey]}`] = [(0, helper_1.resolveMainFilePath)(filePath.replace(node_path_1.default.extname(filePath), ''))];
                });
            }
        });
        this.entry = entryObj;
