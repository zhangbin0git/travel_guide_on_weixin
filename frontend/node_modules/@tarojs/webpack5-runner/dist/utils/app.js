...
class AppHelper {
    ...
    get appEntry() {
        ...
        if (!appEntry)
            return node_path_1.default.join(sourceDir, entryFileName);
        if (Array.isArray(appEntry)) {
            return appEntry.filter(item => node_path_1.default.basename(item, node_path_1.default.extname(item)) === entryFileName)[0];
        }
        else if (Array.isArray(appEntry.import)) {
            return appEntry.import.filter(item => node_path_1.default.basename(item, node_path_1.default.extname(item)) === entryFileName)[0];
        }
        return appEntry;
    }
    get appConfig() {
    ...
    get pages() {
        if (!__classPrivateFieldGet(this, _AppHelper_pages, "f")) {
            ...
            __classPrivateFieldSet(this, _AppHelper_pages, new Set([
                ...appPages.map(item => ({
                    name: item,
                    path: (0, helper_1.resolveMainFilePath)(node_path_1.default.join(sourceDir, item), frameworkExts)
                }))
            ]), "f");
            this.getSubPackages();
        }
            ...
            subPackages.forEach(item => {
                if (item.pages && item.pages.length) {
                    ...
                    item.pages.forEach(page => {
                        ...
                        if (!hasPageIn) {
                            const pagePath = (0, helper_1.resolveMainFilePath)(node_path_1.default.join(sourceDir, pageItem), frameworkExts);
                            this.pages.add({
                                name: pageItem,
                                path: pagePath
                            });
